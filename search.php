...
if(isset($_GET['search_query'])) {
    ...
    $select_product = mysqli_query($con, "SELECT * FROM products WHERE name LIKE '%$search_query%' AND price >= $min_price AND price <= $max_price") or die('query failed');
} else {
    $category_id = $_GET['category_id'] ?? null; // using null coalescing operator to avoid undefined variable notice
    $min_price = isset($_GET['min_price']) && !empty($_GET['min_price']) ? $_GET['min_price'] : 0;
    $max_price = isset($_GET['max_price']) && !empty($_GET['max_price']) ? $_GET['max_price'] : PHP_INT_MAX;
    $select_product = mysqli_query($con, "SELECT * FROM products" . ($category_id ? " WHERE category_id = '$category_id'" : "") . " AND in_stock='Yes'" . ($min_price ? " AND price >= $min_price" : "") . ($max_price ? " AND price <= $max_price" : "")) or die('query failed');
}
